  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const results = await db.select({
        ...
        enrollmentId: documents.enrollmentId,
        type: documents.type,
        fileName: documents.fileName,
        mimeType: documents.mimeType,
        uploadedAt: documents.uploadedAt,
        // Don't return fileData in list view to save bandwidth
      }).from(documents)
    ...
    if (req.method === 'POST') {
      ...
      if (!studentId || !enrollmentId || !type || !fileData) {
      ...
      const existing = await db.select().from(documents)
        .where(and(
          ...
          eq(documents.enrollmentId, enrollmentId),
      ...
      if (existing.length > 0) {
        ...
        const [updated] = await db.update(documents)
          .set({ fileName, fileData, mimeType, uploadedAt: new Date() })
